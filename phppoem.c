...
/* If you declare any globals in php_phppoem.h uncomment this:
ZEND_DECLARE_MODULE_GLOBALS(phppoem)
*/
...
static int compile_view(php_stream *stream, char *compile_path){
	zval *replace_val;
	char *result;
	int  result_len, i;
...
static void exec_view(zend_file_handle *file_handle, zval *assign) {
	zend_op *op_array;
...
PHP_METHOD(poem_view,assign_extract){
	zval *assign, *funname, *retval;
	zval *param[1]; 
	assign = zend_read_property(FETCH_THIS, ZEND_STRL("assign"), 0 TSRMLS_CC);
	// MAKE_STD_ZVAL(tmp);
	// zend_hash_internal_pointer_reset(Z_ARRVAL_P(assign));
	...
	// 	if( zend_hash_get_current_key_ex(Z_ARRVAL_P(assign), &key, &key_len, &idx,0 ,NULL) != HASH_KEY_IS_STRING ) continue;
	// }
	ZVAL_STRING(funname, "extract", 1); 
	param[0] = assign;
	call_user_function(EG(function_table), NULL, funname, 1, retval, param TSRMLS_CC);
	RETURN_TRUE;
}
// ---------- 实现函数End
...
const zend_function_entry phppoem_functions[] = {
	...
	PHP_ME(poem_view, assign_extract, NULL, ZEND_ACC_PUBLIC)
	PHP_ME(poem_view, display, arg_poem_view_display, ZEND_ACC_PUBLIC)
	PHP_FE_END	/* Must be the last line in phppoem_functions[] */
};
...
PHP_MINIT_FUNCTION(phppoem){
	...
	INIT_CLASS_ENTRY(poem, "poem_view", phppoem_functions);
	phppoem_ce = zend_register_internal_class_ex(&poem, NULL, NULL TSRMLS_CC);
	// #define ZEND_STRL(str)  (str), (sizeof(str)-1)
	zend_declare_property_string(phppoem_ce, ZEND_STRL("template_dir"), "", ZEND_ACC_PUBLIC TSRMLS_CC);
	zend_declare_property_string(phppoem_ce, ZEND_STRL("compile_dir"), "", ZEND_ACC_PUBLIC TSRMLS_CC);
	return SUCCESS;
}
